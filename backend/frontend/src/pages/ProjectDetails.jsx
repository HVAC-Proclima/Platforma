...
export default function ProjectDetails() {
  ...
  // ===== deviz export modal =====
  const [showDevizExport, setShowDevizExport] = useState(false)
  ...
  function devizTitle() {
    const pName = project?.title || `Lucrare #${projectID}`
    const cName = client?.name || `Client #${clientID}`
    return `Deviz lucrare - ${pName} - ${cName}`
  }

  function devizFileBase() {
    const pName = (project?.title || `lucrare_${projectID}`).replace(/[\s\/\\]+/g, "_")
    const cName = (client?.name || `client_${clientID}`).replace(/[\s\/\\]+/g, "_")
    const d = new Date()
    const dd = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`
    return `deviz_${cName}_${pName}_${dd}`
  }

  function devizRows() {
    // export exact cum este in tabel (sortare identica)
    const rows = Array.isArray(filtered) ? filtered : []
    return rows.map((it) => ({
      material: String(it?.name ?? ""),
      qty: Number(it?.qty ?? 0),
      um: String(it?.unit ?? ""),
  ...
  function devizTotals(rows) {
    let tQty = 0
    let tCost = 0
    for (const r of rows) {
      tQty += Number(r.qty || 0)
      tCost += Number(r.cost || 0)
    }
    return { tQty, tCost }
  }

  function exportDevizExcel() {
    const rows = devizRows()
    const { tQty, tCost } = devizTotals(rows)
    const sheetRows = [
      [devizTitle()],
      [`Generat: ${new Date().toLocaleString("ro-RO")}`],
      [],
      ["Material", "Cantitate", "UM", "Pret unitar (RON)", "Valoare (RON)"],
      ...rows.map((r) => [r.material, r.qty, r.um, r.unit_price, r.cost]),
      [],
      ["TOTAL", tQty, "", "", tCost],
    ]
    ...
    XLSX.writeFile(wb, `${devizFileBase()}.xlsx`)
    setShowDevizExport(false)
  }
  ...
  function exportDevizPDF() {
    ...
    const title = stripDiacritics(devizTitle())
    doc.setFontSize(18)
    doc.text(title, 40, 40)

    doc.setFontSize(11)
    doc.text(stripDiacritics(`Generat: ${new Date().toLocaleString("ro-RO")} • Elemente: ${rows.length}`), 40, 62)

    const body = rows.map((r) => [
      stripDiacritics(r.material),
      String(Number(r.qty || 0).toFixed(2)),
      stripDiacritics(r.um),
      String(Number(r.unit_price || 0).toFixed(2)),
      String(Number(r.cost || 0).toFixed(2)),
    ])
    ...
    doc.setFontSize(12)
    doc.text(stripDiacritics(`TOTAL cantitate: ${tQty.toFixed(2)}    TOTAL valoare: ${tCost.toFixed(2)} RON`), 40, y)

    doc.save(`${devizFileBase()}.pdf`)
    setShowDevizExport(false)
  }
  ...
  async function removeWorker(w) {
    ...
    try {
      ...
      showToast(setToast, "warn", "Scos din interfață.")
    } finally {
  ...
  async function saveWorkerNote(e) {
    ...
    try {
      ...
      showToast(setToast, "warn", "Notă actualizată.")
    } finally {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button
          onClick={() => setShowDevizExport(true)}
                className="
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.65)" }}>
          <div className="w-full max-w-md rounded-2xl p-5 border border-white/10" style={{ background: "var(--panel)" }}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowDevizExport(false)} className="px-2 py-1 rounded bg-black" type="button">
                ✕
              </button>
            </div>

            <div className="text-sm opacity-80">
              <div className="font-semibold">{project?.title || `Lucrare #${projectID}`}</div>
              <div className="opacity-70">{client?.name || `Client #${clientID}`}</div>
              <div className="mt-2 text-xs opacity-60">Exportul folosește exact rândurile din tabel (cu sortarea curentă).</div>
            </div>

            <div className="mt-4 grid grid-cols-2 gap-2">
              
              <button
                onClick={exportDevizExcel}
                className="
                  group flex items-center justify-center gap-2
                  px-4 py-1 rounded-lg text-lg
                  font-medium
            ...
            <button
              type="button"
              className="mt-3 w-full px-3 py-2 rounded-xl bg-black"
              onClick={() => setShowDevizExport(false)}
            >
              Închide
            </button>
          </div>
